...
half4 colorBlend(half4 bottomColor, half4 topColor) {
  half a_0 = topColor.a + bottomColor.a * (1 - topColor.a);
  half3 c_0 = topColor.rgb + (bottomColor.rgb * (1 - topColor.a));
  return half4(c_0, a_0);
}

half4 tintedLayerSample(SwiftUI::Layer layer, float2 coord, half4 underlensTint) {
    half4 s = layer.sample(coord);

    if (underlensTint.a <= half(0.0))
        return s;

    half a = s.a;
    half3 tintRGB = underlensTint.rgb;
...
half4 getBlurredColor(float2 coord,
                      float blurRadius,
                      SwiftUI::Layer layer,
                      half4 underlensTint) {
    ...
    // Dynamic kernel size based on blur radius
    int kernelSize = int(ceil(blurRadius)) + 1;
    kernelSize = min(kernelSize, 8); // Cap at 8 for performance

    // Use blurRadius directly as sigma (avoid division by zero)
    float sigma = max(blurRadius, 0.01);
    ...
    for (int x = -kernelSize; x <= kernelSize; x++) {
        for (int y = -kernelSize; y <= kernelSize; y++) {
            ...
            color += tintedLayerSample(layer, coord + offset, underlensTint) * weight;
            totalWeight += weight;
        }
    }
...
[[ stitchable ]] half4 glassDistortUnderLens3(float2 position,
                                              SwiftUI::Layer layer,
                                              float2 lensCenter,
                                              float2 lensRadii,
                                              float progress) {
    float2 glassCenter = position - lensCenter;
    float r = lensRadii.y;
    float size = min(lensRadii.x, lensRadii.y) * 2.0;
    float inversedSDF = -sdf(glassCenter, lensRadii, r) / size;

    if (inversedSDF < 0.0)
        return layer.sample(position);

    float distortionEdge = 0.2;
    float distFromCenter = 1 - clamp(inversedSDF / distortionEdge, 0.0, 1.0);

    float distortion = 1.0 - sqrt(1.0 - pow(distFromCenter, 2.0));

    float eps = 1.0;
    float2 grad = float2(
        sdf(glassCenter + float2(eps, 0), lensRadii, r) - sdf(glassCenter - float2(eps, 0), lensRadii, r),
        sdf(glassCenter + float2(0, eps), lensRadii, r) - sdf(glassCenter - float2(0, eps), lensRadii, r)
    );
    float2 normalizedDir = normalize(grad);

    float2 offset = distortion * normalizedDir * size * 0.5;

    float minifyStrength = saturate((1.0 - inversedSDF) * 0.35 / max(progress, 0.001));
    float2 minifyOffset = glassCenter * minifyStrength;

    float2 glassColorCoord = position - offset + minifyOffset;

    float blurIntensity = mix(20, 1, pow(max(progress, 0.001), 2.0));
    float blurEdge = mix(1, distortion, pow(max(progress, 0.001), 5.0));
    float blurRadius = blurIntensity * blurEdge;

    half4 originalColor = getBlurredColor(glassColorCoord, blurRadius, layer, 0);
    half4 glassColor = originalColor;

    // Glow parameters
    float glowWidth = 0.05;
    float glowIntensity = 0.05;
    half3 glowColor = half3(1.0, 1.0, 1.0);  // white glow

    // Glow peaks at edge, fades inward
    float glowFactor = smoothstep(glowWidth, 0.0, inversedSDF);
    glowFactor = pow(glowFactor, 2.0);  // sharpen falloff

    glassColor.rgb += glowColor * glowFactor * glowIntensity;

    float edgeFactor = smoothstep(0.15, 0.0, inversedSDF) * 0.5;
    float2 dir = glassCenter / lensRadii;

    // Sine gives smooth wave across quadrants
    ...
    half brightenAmount = half(1 - progress * 0.85);
    glassColor.rgb = mix(glassColor.rgb, half3(glassColor.a), brightenAmount);

    half4 inactiveWhite = half4(1, 1, 1, 1);
    glassColor = mix(inactiveWhite, glassColor, progress);

    float topOnlySDF = max(sdf(glassCenter, lensRadii, r), -glassCenter.y);
    float softTopOnlySDF = 1 - smoothstep(lensRadii.y, 0.0, topOnlySDF);
    half4 shadow = half4(0,0,0,softTopOnlySDF * 0.12 * progress);
    glassColor.rgb *= (1.0h - shadow.a);
    glassColor.a = glassColor.a + shadow.a * (1.0h - glassColor.a);

    // Physical edge highlight - top left (dark purple)
    float2 lightDir = normalize(float2(1, 1));
    float edgeHighlight = dot(normalizedDir, lightDir);
    edgeHighlight = saturate(-edgeHighlight);

    float edgeMask = smoothstep(0.1, 0.0, inversedSDF);

    float highlightStrength = edgeHighlight * edgeMask;
    highlightStrength = pow(highlightStrength, 2.0);

    // Dark purple tint
    half3 purpleTint = half3(0.5h, 0.2h, 0.4h);
    half highlightIntensity = half(highlightStrength * 0.3 * progress);

    // Darken and tint instead of brighten
...
[[ stitchable ]] half4 sliderThumb(float2 position,
                                   SwiftUI::Layer layer,
                                   float2 lensCenter,
                                   float2 lensRadii,
                                   float progress) {
...
//    return half4(1, 1, 1, 1);
    ...
    float2 glassColorCoord = position - offset;

    float blurIntensity = mix(20, 5, pow(max(progress, 0.001), 2.0));
    float blurEdge = mix(1, distortion, pow(max(progress, 0.001), 5.0));
    float blurRadius = blurIntensity * blurEdge;
...
//    glassColor.rgb = shiftHueToward(glassColor.rgb, glassColor.a, targetHue, edgeFactor);
    ...
    float topOnlySDF = max(sdf(glassCenter, lensRadii, r), -glassCenter.y);
    float softTopOnlySDF = 1 - smoothstep(lensRadii.y, 0.0, topOnlySDF);
    half4 shadow = half4(0,0,0,softTopOnlySDF * 0.03 * progress);
    glassColor.rgb *= (1.0h - shadow.a);
    glassColor.a = glassColor.a + shadow.a * (1.0h - glassColor.a);
//    return half4(softTopOnlySDF, 0, 0, softTopOnlySDF);
    ...
    // Dark purple tint
    half3 purpleTint = half3(0.3h, 0.2h, 0.4h);
    half highlightIntensity = half(highlightStrength * 0.5 * progress);
